...
static char help[] = "PETSc Poisson solver benchmark\n\n";
...
int main(int argc, char** argv) {
    ...
    if (myid == 0) {
        std::cout << "===========================================\n";
        std::cout << "PETSc CPU Poisson Solver Benchmark\n";
        std::cout << "===========================================\n\n";
    ...
    if (myid == 0) {
        config_reader.print_summary("PETSc-CPU");
    }
    
    // 从配置文件获取参数
    std::vector<int> grid_sizes = config_reader.get_grid_sizes();
    std::string precond_type = config_reader.get_petsc_preconditioner(false); // false = CPU
    ...
    for (int grid_size : grid_sizes) {
        ...
        if (myid == 0) {
            ...
            result.test_name = "Poisson_2D_CG_GAMG";
    ...
    if (myid == 0) {
        ...
        writer.write_csv("petsc_results.csv");
        writer.write_json("petsc_results.json");
        std::cout << "\nPETSc benchmark completed!\n";
    }
