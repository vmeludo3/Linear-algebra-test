...
int main(int argc, char** argv) {
    ...
    if (myid == 0) {
        std::cout << "===========================================\n";
        std::cout << "HYPRE CPU Poisson Solver Benchmark\n";
        std::cout << "===========================================\n\n";
    ...
    if (myid == 0) {
        config_reader.print_summary("HYPRE-CPU");
    }
    
    // 从配置文件获取参数
    std::vector<int> grid_sizes = config_reader.get_grid_sizes();
    std::string precond_type = config_reader.get_hypre_preconditioner(false); // false = CPU
    ...
    for (int grid_size : grid_sizes) {
        ...
        if (myid == 0) {
            ...
            result.library_name = "HYPRE";
            result.test_name = "Poisson_2D_PCG_BoomerAMG";
    ...
    if (myid == 0) {
        ...
        writer.write_csv("hypre_results.csv");
        writer.write_json("hypre_results.json");
        std::cout << "\nHYPRE benchmark completed!\n";
    }
